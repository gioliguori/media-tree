...
function cleanupConnection() {
	...
	// Step 1: Close PeerConnection FIRST
	if (pc) {
		try {
			pc.close();
			pc = null;
	...
	// Step 2: DELETE with keepalive (works even when page is closing)
	if (resource) {
		try {
			fetch(backend + resource, {
				method: 'DELETE',
				headers: token ? { 'Authorization': 'Bearer ' + token } : {},
				keepalive: true  // KEY: ensures request completes even if page closes
